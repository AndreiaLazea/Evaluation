            ...
            {data.length > 0 ? (
                data.map(artist => (
                    <div key={artist._id} className="data-item">
                        <h2>{artist.name}</h2>
                        <button onClick={() => handleUpdateClick({ ...artist, type: 'artist' })}>Update Artist</button>
                        {artist.albums.map(album => (
                            <div key={album._id} className="album-item">
                                <h3>{album.title}</h3>
                                <p>{album.description}</p>
                                <button onClick={() => handleUpdateClick({ ...album, artistId: artist._id, type: 'album' })}>Update Album</button>
                                <ul>
                                    {album.songs.map(song => (
                                        <li key={song._id}>
                                            <strong>{song.title}</strong> - {song.length}
                                            <button onClick={() => handleUpdateClick({ ...song, artistId: artist._id, albumId: album._id, type: 'song' })}>Update Song</button>
                                        </li>
                                    ))}
                                </ul>
                            </div>
